     ...
     if(nameIn==0){
       ...
       if(GameOver==0){
         ...
         if(gamePhase==0){
           ...
           if(delay>150){
             ...
             stroke(255,255);
             fill(255,150);
             if((mouseX<3*width/4 && mouseX>width*1/4 && mouseY>(height-height/3) && mouseY<(height-height/6))){
           ...
           if(delay>30){ //Delay the ability to exit the selection screen so the previous click does not trigger it accidentally
             ...
             if(keyIsPressed || (mouseIsPressed && (mouseX>3*width/4 || mouseX<width*1/4 || mouseY<(height-height/3) || mouseY>(height-height/3+height/9)))) {
               mouseIsPressed=false;
               keyIsPressed=false;
               //fullscreen(full);
               //resizeCanvas(displayWidth, displayHeight);
           ...
           if(level<3){
             ...
             image(imgSeven,0,height/2-width/16.59,width/6,width/8.29);
           }
           else{
             ...
             for (let i = 0; i < OneBH.length; i++) {
               ...
               if(level==2){
                 ...
                 if(i%2==0 ){
                   ...
                   BHx=height/2*tan(radians(delay/1.2+i*72))*cos(radians(delay/1.2+i*72))+width/1.5;
                   BHy=height/1.4*cos(radians(delay/1.2+i*72))*sin(radians(delay/1.2+i*72))+height/2;
                 }
                 else{
                   BHx=-200;
                   BHy=i*width/10;
                 ...
                 let yyyyyy=(height/1.2)*sin(radians(angle))*cos(radians(angle))+height/2;
                 BHx=xxxxxx;
                 BHy=yyyyyy;
               }
               else if(level==9){
                 ...
                 let xxxxxxx=(height/12+width/12)*cos(radians(angle))-width/5*cos((height/width+radians(angle))*radians(angle))+width/1.4;
                 let yyyyyyy=(height/12+width/12)*sin(radians(angle))-width/5*sin((height/width+radians(angle))*radians(angle))+height/2;
                 BHx=xxxxxxx*0.85;
                 BHy=yyyyyyy*1;
               }
               else if(level==10){
                 ...
                 let xxxxxxxx=(height/15)*(cos(radians(angle))+radians(angle)*sin(radians(angle)))+width/2;
                 let yyyyyyyy=(height/15)*(sin(radians(angle))-radians(angle)*cos(radians(angle)))+height/2;
                 BHx=xxxxxxxx;
                 BHy=yyyyyyyy;
               }
       ...
       //What happens after being eaten by a black hole or destroyed by an asteroid, have different sequences that occur
         ...
         if(lifeCount==0){
           ...
           noStroke();
           fill(0,0,0,255);
           rect(width/2+width*cos(radians(delay)/5)/2,0,height/10,height);
           rect(width/2+width*sin(radians(delay)/5)/2,0,height/10,height);
           rect(width/5+width*sin(radians(delay)/3)/2,0,height/10,height);
